...
public:
	...
	PROP_PARAM(Vec3, left_handy_pos_vr);
	PROP_PARAM(Vec3, left_handy_rot_euler_vr);
	PROP_PARAM(Vec3, right_handy_pos_vr);
	PROP_PARAM(Vec3, right_handy_rot_euler_vr);
...
private:
	...
	Unigine::Math::quat handy_rot, left_handy_rot_vr, right_handy_rot_vr;
	...
	quat get_vr_rotation(int hand_num);
	vec3 get_vr_pos(int hand_num);
	vec3 original_scale;

	// callbacks and actions
	UNIGINE_INLINE void position_callback(Unigine::BodyPtr callback_body) { last_pos_index = 1 - last_pos_index; last_pos[last_pos_index] = callback_body->getPosition(); }
	UNIGINE_INLINE void contact_callback(Unigine::BodyPtr callback_body, int num) { onContact(callback_body, num); }
	UNIGINE_INLINE void frozen_callback(Unigine::BodyPtr callback_body) { onFrozen(callback_body); }
